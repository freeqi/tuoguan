<template>
  <div class="message_container publish_container overflowHidden">
        ...
        <div class="content_box">
          <div class="priceChange_box" v-show="article.msgTypeId !== 1">
            <div class="add_btn_group">
              <Button type="primary" @click="addGoods">新增调价物品</Button>
    ...
    <Modal v-model="goodsFlag" class="goods_data" :mask-closable="false" width="1000">
      ...
      <div slot="footer">
        <Button type="primary" @click="saveAddGoods('formValidateRef')">{{title === '新增' ? '新增' : '修改'}}</Button>
...
export default {
  data () {
    return {
      changedFlag: false, //在发布前任何一个改动都需要重新生成文本
      options1: {
        disabledDate (date) {
      ...
      table_column_price: [
        ...
        {title: '销售价', key: 'upSalePrice', width: 65},
        {title: '调整售价', key: 'salePrice', width: 65},
  ...
  computed: {
    ...
    goodsFilter () {
      ...
      if (this.goodsList) {
        data = toFilterKey(
          this.goodsList,
          'medicalItemName,medicalItemCode,aliasName,manufacturer,packaging,mnemonic',
          this.modalSearch
        )
      } else {
        data = []
  ...
  methods: {
    ...
    creatText () {
      if (this.table_data_price.length === 0) {
        ...
        return false
      } else if (!this.disposeTime) {
        this.$Message.error('请选择调价截止日期！')
        return
      }
      let title = `<h1 style="text-align: center;">调整公告</h1>`
      let txtLi = this.table_data_price.map((res, index) => {
        return `<li>${index + 1}.物品:<span style="font-style: italic;">${res.itemName}</span>&nbsp; &nbsp;` +
          ...
          `<span style="font-weight: bold;">原销售价格</span><span style="font-weight: bold;">：${res.upSalePrice}元</span>，` +
          `<span style="font-weight: bold;">调整为：${res.salePrice} 元</span>。</li>`
      }).join('')
      let time = `<h3 style="text-align: right;">调价截止日期：${formateDateToString(this.disposeTime, 'yyyy-MM-dd')}</h3>`
      this.$refs.editor.setHtml(`<ul>${txtLi}</ul>${time}`)
      this.article.msgContent = this.$refs.editor.getHtml()
    ...
    delBtn (row, index) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.table_data_price.splice(index, 1)
          this.changedFlag = false
        }
      })
    },
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.title === '新增') {
            ...
            // console.log(this.table_data_price, params)
            // 这样不能实时刷新数据
            // this.table_data_price[this.editIndex] = params
            this.table_data_price.splice(this.editIndex, 1)
            this.table_data_price.splice(this.editIndex, 0, params)
          }
    ...
    selectOneInfo (item) {
      ...
      this.formValidate.medicalId = id
      this.formValidate.itemName = itemName
      if (this.title === '修改') {
        let {medicalId, itemName, purchasingPrice, salePrice, upSalePrice, remark} = {...item}
        this.formValidate = {
          medicalId, itemName, purchasingPrice, salePrice, upSalePrice, remark
        }
      }
      this.modalSearch = ''
      this.goodsFlag = true
          ...
          .then(function (res) {
            if (res.data.success) {
              ...
              for (let i in result) {
                ...
                // 物品名 + 别名 + 产品规格 + 生产厂家 + 包装规格 +助记码 + 编码
                item.itemName = `${result[i].medicalItemCode ? result[i].medicalItemCode : ''} ` +
                  `${result[i].medicalItemName} ` +
                  `${result[i].brand ? '(' + result[i].brand + ')' : ''} ` +
                  `${result[i].procurementPackage ? '(' + result[i].procurementPackage + ')' : ''} ` +
                  `${result[i].aliasName ? '(' + result[i].aliasName + ')' : ''} ` +
                  `${result[i].manufacturer ? '(' + result[i].manufacturer + ')' : ''} ` +
                  `${result[i].packaging ? result[i].packaging : ''} [${result[i].mnemonic}]`
                item.id = result[i].id
                // cy 用于模糊搜素
                item.medicalItemName = result[i].medicalItemName // 物品名称
                item.medicalItemCode = result[i].medicalItemCode // 编码
      ...
      this.swsApi
        ...
        .then(res => {
          ...
          if (data.success) {
            ...
            if (id === 1) {
              this.treeData1 = data.result.filter(res => {
                // 去掉人员（去掉children里的数据），只显示机构
    ...
    publish () {
      ...
      if (msgTypeId === 2 && this.disposeTime === '') {
        ...
        this.hasError = true
        return false
      } else if (msgContent === '<p><br></p>' || msgContent === '') {
        ...
        this.$Message.error('数据有改动，请重新进行文本生成操作！')
        return false
      }
      let params = {
        msgContent,
      ...
      if (msgTypeId === 2) {
        params.noticeMedicalInputs = this.table_data_price
        params.outTime = new Date(this.disposeTime.setHours(this.disposeTime.getHours() + 8))
      }
      this.swsApi
