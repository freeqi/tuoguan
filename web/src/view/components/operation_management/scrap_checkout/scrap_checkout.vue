<template>
  <div id="purchase_requisition" :class="{ overflow : detailShowFlage}">
    <div class="btn-groups">
      ...
      <span>订单状态</span>
      <Select transfer v-model="orderCheckedId" placeholder="请选择订单状态" style="width: 120px;" @on-change="changeOrderType" filterable>
        <Option :value="item.id" v-for="item in orderType" :key="item.id">{{item.txt}}</Option>
      </Select>
      <span>时间范围</span>
      <DatePicker v-model="time" format="yyyy-MM-dd" type="daterange" placement="bottom-end" placeholder="请选择时间段" style="width: 200px;" @on-change="changeDate">
      </DatePicker>
      <Input v-model="searchKey" @on-search="search" search enter-button placeholder="请输入关键字" style="width: 200px; display: inline-table;" />
...
export default {
      ...
      table_columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return (
              <span>
                {params.row.groupAuditDate &&
                  new Date(params.row.groupAuditDate)
                    .toLocaleDateString()
                    .replace(/\//g, "-")}
              </span>
            );
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            if (status === "2" || !status) {
              ...
              if (
                params.row.warehouseId == "2543420c3132403098b85cad056c577e" &&
                !params.row.auditorLevel
              ) {
                return <tag color="default">未估价</tag>;
              } else {
  ...
  methods: {
    refresh() {
      ...
      this.swsApi
        .swsPost(
          "CenterDocking/CollectGetCollectGetMaterialApplyApprove",
          params
        )
        .then((res) => {
          if (res.data.success) {
    ...
    getFormList(pageNum = 1) {
      ...
      this.loading = true;
      this.swsApi
        .swsPost("CenterDocking/GetMapprove", params)
        .then((res) => {
          if (res.data.success) {
            this.table_data = res.data.result;
            this.dataCount = res.data.dataCount;
