...
export default {
  ...
  computed: {
    ...
    headerFlag () {
      if (this.hospitalCheckedIds.length === 1 && this.hospitalCheckedIds[0] !== '0') {
        return false
  ...
  methods: {
    ...
    changeHospitals (ids) {
      ...
      this.tableData = []
      this.getTableDataByKey(this.tabCheckedId)
      _debounce(this.getTableDataByKey, this.tabCheckedId, 500)
    },
    // 切换医院
    changeHospital (id) {
    ...
    getDayReportData () {
      ...
      this.swsApi
        .swsPost(
          '/BusinessTargetClntroller/BusinessTarget/DayReportAsync', params)
        .then(res => {
          if (res.data.success) {
