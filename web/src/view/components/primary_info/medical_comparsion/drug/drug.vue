  ...
  <div id="medical_comparison">
    <div class="top">
      ...
      <div class="button-group">
        ...
        <Button type="success" @click="compare" v-permission="buttonRole.YPZD_DZ" v-show="tabValue!=='医保更新数据'">对照</Button>
        <Button type="default" @click="cancelCompare" v-show="tabValue === '已对照项目'" v-permission="buttonRole.YPZD_QXDZ">取消对照</Button>
        <Button type="primary" @click="update">更新药品目录</Button>
        <Button type="primary" @click="updateMl" :loading="updateLoad">更新限价目录</Button>
      </div>
      <Divider/>
    </div>
    <div class="content">
      <Tabs type="card" class="tab" @on-click="changeTab" :value="tabValue">
        ...
        <!-- <TabPane label="医保更新数据" name="医保更新数据">
        </TabPane> -->
      </Tabs>
    </div>
  </div>
...
export default {
  ...
  methods: {
    update(){
      this.swsApi.swsPost("Data/SI/GetYPML").then(res=>{
        if (res.data.success) {
    ...
    updateMl(){
      ...
      this.swsApi.swsPost("Data/SI/XJML").then(res=>{
        if (res.data.success) {
    ...
    setComparsionRowColor (row) {
      if (this.local_data.length !== 0) {
        let arr = this.local_data.filter(item => {
          // return item.hiCenterCode !== null && item.hiCenterCode === row.yplsh
          return item.nationItemCode !== null && item.nationItemCode === row.gjypdm
        })
    ...
    setOption (curRow) {
      ...
      if (curRow.hasOwnProperty('bgsj')) { // 看数据为区分医保中心目录
        let { ycmc, hl, hldw, bzsl, bzdw, rl, rldw, ylfydj, yplsh, spm, ylbzdj, id, tym, gjypdm ,zxzjdw} = curRow
        ...
        this.compareInfo.si_YpmlQueryInput.gjypdm = gjypdm//国家项目编码
        this.centerItemId = id
      } else {
        let { hiCenterCode,purchasingPrice, hiLevel, manufacturer, goodsName, minDose,packaging, id, medicalItemName,nationItemCode} = curRow
        ...
        this.compareInfo.medicalItemInput.packaging = packaging
        ...
        if (this.tabValue === '已对照项目') {
          let data = this.ydzList.filter(res => {
            ...
            return res.gjypdm === nationItemCode
          })
          if (data.length > 0) {
            let { ycmc, hl, hldw, bzsl, bzdw, rl, rldw, ylfydj, yplsh, spm, ylbzdj, id, tym, gjypdm ,zxzjdw} = data[0]
            this.compareInfo.si_YpmlQueryInput.manufacturer = ycmc
            this.compareInfo.si_YpmlQueryInput.packaging = `${rl ? rl + rldw + ':' : ''}${hl}${bzsl}${zxzjdw}/${bzdw}`
            // this.compareInfo.si_YpmlQueryInput.yplsh = yplsh
            this.compareInfo.si_YpmlQueryInput.ylfydj = ylfydj
