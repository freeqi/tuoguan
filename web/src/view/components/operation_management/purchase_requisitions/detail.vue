    ...
    <div class="purchase_detail" v-show="purchase_detail">
        ...
        <p class="record">
          ...
          <span class="right button-group">
            ...
            <Button
              type="default"
              class="approval"
              v-permission="buttonRole.CGXQ_SHENPI"
            ...
            <Button
              type="success"
              v-permission="buttonRole.CGXQ_DJ"
              v-show="orderDetail.dataState === 7"
              :loading="cspLoading"
              @click="confirmSalePriceBtn"
              >定价</Button
            >
            <Button
              type="success"
              v-permission="buttonRole.CGXQ_TS"
              v-show="isShowPushCenterBtn"
              @click="pushBtn"
              >推送</Button
            >
            <Button
              type="info"
              v-permission="buttonRole.CGXQ_CD"
              @click="splitOrder"
              v-show="supplierSets.size > 1"
              >拆单</Button
            >
            <Button
              type="primary"
              v-permission="buttonRole.CGXQ_DC"
              @click="exportDetailTable"
              >导出</Button
            >
            <Button type="default" @click="hide" icon="md-undo"
              >返回</Button
            ></span
          >
        </p>
        ...
        >
          <template slot-scope="{ row, index }" slot="supplierName">
            <Select
              ...
              filterable
              v-if="
                editIndex === index && (editAllAccessType || editSomeAccessType)
              "
            >
              <Option
                :value="item.id"
          ...
          <template slot-scope="{ row, index }" slot="salePrice">
            <InputNumber
              ...
              v-if="
                editIndex === index && (editAllAccessType || yiBaoAccessType)
              "
            ></InputNumber>
            <span v-else-if="row.id">
              <template>
                <span v-if=" row.socialSecurityPrice !== null && row.dualPrice !== null "
                ...
                <sub v-show="row.isSellFree" style="color:red">(售价可为0)</sub>
              </template>
            </span>
          </template>
          ...
          <template slot-scope="{ row, index }" slot="sumPrice">
            <InputNumber
              ...
              v-if="editIndex === index && editAllAccessType"
            ></InputNumber>
            <span v-else :class="{ emphasize: !row.id }">{{
              row.sumPrice
            }}</span>
          </template>
          ...
          <template slot-scope="{ row, index }" slot="monthAverage">
            <InputNumber
              ...
              v-if="editIndex === index && !isFixedType && editAllAccessType"
            ></InputNumber>
        ...
        <Form :label-width="60" style="margin-bottom: 10px;">
          ...
          <FormItem label="审核意见" style="margin-bottom: 10px">
            <Input
              ...
              :rows="4"
              placeholder="请输入审核意见..."
              v-model="approval.advice"
            />
          </FormItem>
        </Form>
        <div slot="footer" class="center">
          <Button type="primary" v-show="!saveLoading" @click="saveApproval"
            >审核</Button
          >
          <Button type="primary" v-show="saveLoading" :loading="true"
            >审核中</Button
          >
      ...
      <Modal v-model="closeModal" @on-cancel="formEdit.remark = ''">
        <p slot="header" style="text-align: center;">
          确定要关闭该条明细：{{ formEdit.medicalName }} 吗？
        </p>
        <Input
        ...
        />
        <div slot="footer">
          <Button type="error" @click="handleClose" :loading="handleBtn"
            >确认关闭</Button
          >
          <Button
            @click="
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            return params.row.id ? (
              <span>{params.row.dualPrice || 0}</span>
            ) : (
              <span></span>
            );
          }
        },
        {
          ...
          render: (h, params) => {
            if (!(this.isYpHc && !params.row.isHIS)) {
              // 程序计算的与对照的值不相等则显示程序计算值，提示工作人员换算
              if (
                params.row.id &&
                params.row.si_SecurityPrice !== params.row.socialSecurityPrice
              ) {
                // return <i-tooltip props={{ content: params.row.si_SecurityPrice || '/'}}><span style="color:yellow">{params.row.socialSecurityPrice || '/'}</span></i-tooltip>
                return params.row.id ? (
                  <span>
                    {params.row.socialSecurityPrice || "/"}
                    <sup style="color:blue"> {params.row.si_SecurityPrice}</sup>
                  </span>
                ) : (
                  <span></span>
                );
                ...
                return params.row.id ? (
                  <span>{params.row.socialSecurityPrice || "/"}</span>
                ) : (
                  <span></span>
                );
            } else {
  ...
  computed: {
    ...
    // 上级待审 6 和未定价  7 且 修改权限  只改采购定价，供应商
    yiBaoAccessType() {
      return (
        this._filterButton(this.buttonRole.CGXQ_DJ) &&
        this.formDetail.dataState === 7
      );
    },
    editAllAccessType() {
      return (
        this._filterButton(this.buttonRole.CGXQ_XG2) &&
        this.formDetail.dataState === 5
      );
    },
    // editSomeAccessType(){
    ...
    editSomeAccessType() {
      return (
        this._filterButton(this.buttonRole.CGXQ_XG2) &&
        [6, 7, 8].includes(this.formDetail.dataState)
      );
    },
    // 固定资产类型？
    isFixedType() {
    ...
    isYpHc() {
      return (
        this.formDetail.medicalItemType === 1 ||
        this.formDetail.medicalItemType === 2
      );
    },
    currentStep() {
  ...
  watch: {
    formDetail() {
      ...
      // 未推送  8 且 修改权限  只改采购定价，供应商
      let hasEdit =
        this.yiBaoAccessType ||
        this.editSomeAccessType ||
        this.editAllAccessType;
      // 20191231 cy 增加对表单列的过滤显示
      // console.log(`${this.formDetail.dataState},isFixedType:${this.isFixedType}
      // 'editSomeAccessType':${this.editSomeAccessType},editAllAccessType:${this.editAllAccessType},yiBaoAccessType:${this.yiBaoAccessType}`)
      let fliterColumns = this.table_base.filter(res => {
        // 如果订单物品类型为3(5,6,7),4 过滤掉双控价
  ...
  methods: {
    ...
    salePriceVerify() {
      ...
      let data = this.table_data.map(item => {
        if (item.salePrice === 0 && item.dataState!==2 && !item.isSellFree) {
    ...
    confirmSalePrice() {
      ...
      this.swsApi
        .swsGet("CenterDocking/OrderPricing/" + this.formDetail.id)
        .then(res => {
          if (res.data.success) {
    ...
    pushBtn() {
      ...
      this.supplierVerify() &&
        this.$Modal.confirm({
          title: "提示",
          content: `<p>${
            num !== undefined ? "含有销售价为 <strong>零</strong> 的物品，" : ""
          }确定要进行推送操作吗？</p>`,
          onOk: () => {
            this.pushOrder();
          }
    ...
    pushOrder() {
      this.swsApi
        .swsGet(
          "/CenterDocking/OrderDetail/NewPushCenter/" + this.formDetail.id
        )
        .then(res => {
          if (res.data.success) {
        ...
        .then(res => {
          if (res.data.success) {
            ...
            this.table_data = orderDetailData.map(res => {
              ...
              } else if (!res.id && res.currentInventory == 0) {
                res.currentInventory = "";
              }
              return res;
            });
          } else {
