    ...
    <Tabs :value="currentTabPanel" ref="tab" class="tabs" @on-click="changeTab()">
      ...
      <TabPane label="生化检测" name="check">
        <div class="info-box">
          ...
          <div class="detail">
            <RadioGroup v-model="checkType" @on-change="changeT">
        ...
        <Table height="420" :loading="tloading" :columns="table_column" :data="checkData"></Table>
      </TabPane>
    </Tabs>
    <!-- 数据请求完成前显示加载 -->
...
<script>
import {formateDateToString, toFilterKey} from '@/libs/tools'
...
export default {
  ...
  computed: {
    ...
    // dateFilter () {
    //   console.log('dateFilter', this.selectBioType)
    //   console.log('checkData', this.checkData)
    //   let data = []
    //   let filterName = this.detectionTypeLists[this.comptIndex].filter(res => {
    //     return res.index === this.selectBioType
    //   })[0].name
    //   if (this.selectBioType !== 0) {
    //     data = toFilterKey(this.checkData, 'testType', filterName === '全部' ? '' : filterName)
    //   } else {
    //     data = this.checkData
    //   }
    ...
    //   return data
    // }
    // cy 所选生化检测类型对应的类别数组index
    // comptIndex () {
    //   console.log('compIndex', this.checkType, parseInt(this.checkType) - 1)
    //   return parseInt(this.checkType) - 1
    // }
  ...
  watch: {
    checkType () {
      ...
      this.changeBioType()
      // console.log('watch', this.checkType, this.selectBioType, this.comptIndex)
    }
  },
  methods: {
    changeTab () {
      ...
      if (tabName === 'maintenance' && !this.getEkrFlage) {
        ...
        // 生化检测
        // console.log('生化检测', tabName, this.getEbtFlage)
        this.changeT()
      } else {
        return false
      }
    ...
    changeT () {
      ...
      let api = `Equipment/GetEbt/${this.equipmentCheckedId}/${id}`
      this.tloading = true
      this.getDataMethod(api, (res) => {
        if (res.data.success) {
