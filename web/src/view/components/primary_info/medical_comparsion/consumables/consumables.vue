    ...
    <div class="top">
      ...
      <Form ref="compareH" :model="compareInfo.medicalItemInput">
        ...
        <Row class="form" :gutter="10">
          ...
          <FormItem label="销售价" prop="salePrice">
            <InputNumber :min="0" style="width: 100%" v-model="compareInfo.medicalItemInput.salePrice" placeholder="请输入"></InputNumber>
          </FormItem>
          </Col>
          <Col :lg="3" :md="3" :sm="3">
          ...
          <FormItem label="生产厂家" prop="manufacturer">
            <Input type="text" v-model="compareInfo.medicalItemInput.manufacturer" placeholder="请输入"></Input>
          </FormItem>
          </Col>
          <Col :lg="2" :md="2" :sm="2">
      ...
      <Form ref="compare" :model="compareInfo.si_YpmlQueryInput">
        <Row class="form" :gutter="10">
          ...
          <Col :lg="2" :md="2" :sm="2">
          <FormItem label="销售价" prop="salePrice">
            <InputNumber :min="0" style="width: 100%" v-model="compareInfo.si_YpmlQueryInput.salePrice" placeholder="请输入"></InputNumber>
          </FormItem>
          </Col>
          <Col :lg="3" :md="3" :sm="3">
          <FormItem label="规格" prop="packaging">
            <Input type="text" v-model="compareInfo.si_YpmlQueryInput.packaging" placeholder="请输入"></Input>
          </FormItem>
          </Col>
          <!-- <Col :lg="3" :md="3" :sm="3">
          ...
          <FormItem label="生产厂家" prop="bz">
            <Input type="text" v-model="compareInfo.si_YpmlQueryInput.bz" placeholder="请输入"></Input>
          </FormItem>
          </Col>
          <Col :lg="2" :md="2" :sm="2">
      ...
      <div class="button-group">
        ...
        <Button type="success" @click="compare" v-permission="buttonRole.HCDZ_DZ">对照</Button>
        <Button type="default" @click="cancelCompare" v-show="tabValue == '已对照项目'" v-permission="buttonRole.HCDZ_QXDZ">取消对照</Button>
        <Button type="primary" @click="update">更新耗材目录</Button>
      </div>
      <Divider />
    </div>
    <div class="content">
      <Tabs type="card" class="tab" @on-click="changeTab" :value="tabValue">
        <TabPane label="本地项目目录" name="本地项目目录">
          <Table ref="table_local" :height="240" :loading="loading" highlight-row :columns="local_column" :data="local_data" no-data-text="暂无数据" @on-current-change="setOption"></Table>
          <h3 class="sec_header">医保中心目录</h3>
          <Table ref="table_center" :height="240" highlight-row :row-class-name="setComparsionRowColor" :loading="center_loading" :columns="center_column" :data="center_data" no-data-text="暂无数据" @on-current-change="setOption"></Table>
        </TabPane>
        <TabPane label="已对照项目" name="已对照项目">
          <Table ref="table_local" :height="480" :loading="compared_loading" highlight-row :columns="compared_column" :data="compared_data" no-data-text="暂无数据" @on-current-change="setOption"></Table>
          <div class="pagination" v-show="dataCount > pageSize">
            <Page :total="dataCount" :page-size="pageSize" :current.sync="pageNum" @on-change="handleChangePage" />
          </div>
        </TabPane>
      </Tabs>
    </div>
...
export default {
  ...
  methods: {
    update() {
      this.swsApi.swsPost("Data/SI/GetHCML").then((res) => {
        if (res.data.success) {
    ...
    setComparsionRowColor(row) {
      if (this.local_data.length !== 0) {
        let arr = this.local_data.filter((item) => {
          // return item.hiCenterCode !== null && item.hiCenterCode === row.xmlsh
          return (
            item.nationItemCode !== null && item.nationItemCode === row.gjxmdm
          );
        });
    ...
    compare() {
      ...
      this.swsApi
        .swsPost(
          `Data/SI/YPMLAsync/${this.localItemId}/${this.centerItemId}/${this.typeID}`
        )
        .then((res) => {
          if (res.data.success) {
            this.$Message.success({
    ...
    setOption(curRow) {
      ...
      if (curRow.hasOwnProperty("bgsj")) {
        let { dw, gg, ylfydj, xmlsh, ylbzj, id, xmmc, gjxmdm, bz } = curRow;

        this.compareInfo.si_YpmlQueryInput.packaging = gg;
        ...
        if (this.tabValue === "已对照项目") {
          ...
          if (data.length > 0) {
            let { dw, gg, ylfydj, xmlsh, ylbzj, id, xmmc, gjxmdm, bz } =
              data[0];
            this.compareInfo.si_YpmlQueryInput.packaging = gg;
            this.compareInfo.si_YpmlQueryInput.yplsh = xmlsh;
            this.compareInfo.si_YpmlQueryInput.ylfydj = ylfydj;
    ...
    searchComparedData() {
      ...
      let {
        ...
        manufacturer,
        nationItemCode,
      } = this.compareInfo.medicalItemInput;
      let params = {
