    ...
    <main class="publish_container_content">
      <div v-show="tabActivedId == 1">
        <div class="message_type">
          <span>请选择类目：</span>
          <RadioGroup v-model="article.msgTypeId" type="button">
            <Radio v-for="item in messageTypeList" :key="item.id" :label="item.id">{{item.txt}}</Radio>
          </RadioGroup>
            <Button type="primary" class="add_price_item_btn">新增调价物品</Button>
        </div>
  ...
  data () {
    return {
      table_column_price: [
        ...
        {title: '物品名称', key: 'name', minWidth: 75},
        {title: '价格', key: 'price1', width: 65},
        {title: '调价', key: 'price2', width: 65}
