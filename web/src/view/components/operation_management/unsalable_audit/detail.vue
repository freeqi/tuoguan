  ...
  <transition name="fade_enter">
    <div class="purchase_detail" v-show="purchase_detail">
      <div class="top">
        <div class="step-wrapper" ref="step">
          <Steps :current="currentStep" :status="currentStatus">
            <Step
              v-for="(item, index) in scrapProcessOutPuts"
              :key="index"
              :title="item.title"
        ...
        <p class="record">
          <strong class="red">{{formDetail.centerName}}</strong>
          <span style="margin-left:0;"> 滞销单号: {{dataDetail.purchaseNo}}
          ...
          <span class="right button-group">
            <Button
              ...
              size="large"
              @click="showApproval"
              v-if="formDetail.groupAuditStatus === '2' || !formDetail.groupAuditStatus"
              v-permission="buttonRole.ZXSH_SHENPI"
            >审批</Button>
            <Button
              type="primary"
              size="large"
              v-permission="buttonRole.ZXSH_DC"
      ...
      <div class="detail_table" ref="detail_table">
        <Table
          ...
          <template slot-scope="{ row, index }" slot="action">
            ...
            <div v-if="row.id !== null && (!formDetail.groupAuditStatus || formDetail.groupAuditStatus === '2')">
              <Tooltip content="修改" placement="top" transfer v-permission="buttonRole.ZXSH_EDIT">
                <Icon
                  type="md-create"
                  size="22"
                  color="#4f95e8"
      ...
      <Modal v-model="approvalModal" className="vertical-center-modal" width="400">
        <p slot="header" align="center">审核滞销单</p>
...
export default {
    ...
    return {
      ...
      table_columns: [
        ...
        // },
        {
          ...
          width: 100,
          render: (h, params) => {
            if(params.row.productionDate=='/') return <span>/</span>
            else return <span>{params.row.productionDate!==null && new Date(params.row.productionDate).toLocaleDateString().replace(/\//g, '-')}</span>
          }
        },
        {
          title: '有效期至',
          ...
          width: 100,
          render: (h, params) => {
            if(params.row.qualityDate=='/') return <span>/</span>
            else return <span>{params.row.qualityDate!==null && new Date(params.row.qualityDate).toLocaleDateString().replace(/\//g, '-')}</span>
          }
        },
        {
          title: '创建时间',
          ...
          width: 100,
          render: (h, params) => {
            return <span>{params.row.founderDate!==null && new Date(params.row.founderDate).toLocaleDateString().replace(/\//g, '-')}</span>
          }
        },
        {
          title: '厂家',
  ...
  methods: {
    ...
    exportDetailTable () {
      ...
      columns.pop()
      let params = {
        filename: `滞销订单${this.formDetail.returnNo}明细表`,
        columns: columns,
        data: datas
      }
    ...
    getFormDetail () {
      ...
      this.loading = true
      this.swsApi
        .swsGet(`CenterDocking/GetMaterialsWarningDetails/${this.formDetail.id}`)
        .then(res => {
          if (res.data.success) {
            let {orderApproveOutPuts,detailOutPuts,applyListOutPut} = res.data.result
            this.table_data = detailOutPuts.map((item, index) => {
              if(item.id) {
                this.computedNum = accAdd(this.computedNum,item.amount)
                this.computedTotalPrice = accAdd(this.computedTotalPrice,item.sumPrice)
              }
              item.no = index + 1
              return item
            })
            this.scrapProcessOutPuts = orderApproveOutPuts || []
            this.dataDetail = applyListOutPut
          } else {
            this.$Message.error(`滞销单详情查询出错，请稍后再试！`)
          }
          this.loading = false
        })
        .catch(e => {
    ...
    saveApproval () {
      ...
      this.saveLoading = true
      this.swsApi
        .swsPost('CenterDocking/MaterialsWarningRequest', this.approval)
        .then(res => {
          if (res.data.success) {
